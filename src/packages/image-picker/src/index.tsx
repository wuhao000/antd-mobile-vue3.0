...
export default defineComponent({
  ...
  setup(props, {emit, slots}) {
    ...
    const fileSelectorInput = computed(() => {
      return fileSelectorInputRef.value as any;
    });
    ...
    const addImage = (imgItem: any[]) => {
      const newImages = props.value ? [...props.value] : [];
      imgItem.forEach(img => {
  ...
  render() {
    ...
    value.forEach((image: any, index: number) => {
      ...
      const imgStyle = {
        backgroundImage: (this.localIsImage(image)) ? `url(${image.url})` : 'none',
